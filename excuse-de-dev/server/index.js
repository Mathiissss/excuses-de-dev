...
app.post('/api/excuses', (req, res) => {
  ...
  if (!message || !tag) {
    return res.status(400).json({ error: 'Message et tag sont requis' });
  }

  const lastCode = Math.max(...excuses.map(e => e.http_code));
  const newExcuse = {
    http_code: lastCode + 1,
    tag: tag,
    message: message
  };
...
app.get('/api/excuses/random', (req, res) => {
  ...
  res.json(excuses[randomIndex]);
});

app.get('/api/excuses/:code', (req, res) => {
...
app.get('/api/health', (req, res) => {
  res.json({ status: 'OK', message: 'API Excuses de Dev fonctionne!' });
});

app.listen(PORT, () => {
  console.log(`Serveur API démarré sur le port ${PORT}`);
  console.log(`${excuses.length} excuses disponibles`);
});